...
void FFTSpectrumAnalyzerAudioProcessorEditor::paint(juce::Graphics& g)
{
	...
	if (audioProcessor.minBlockSize) {
		if (setToLog == true) { 
			plot2.startNewSubPath(std::log10(xStartXYAxis) + xShift, yStartPlot + logScale * std::log10(binMag[1][0]) * scaleY + yShift); 
			plot1.startNewSubPath(std::log10(xStartXYAxis) + xShift, yStartPlot + logScale * std::log10(binMag[0][0]) * scaleY + yShift); 
...
float FFTSpectrumAnalyzerAudioProcessorEditor::getYCoord(int plotNumber, bool log, int index) {
	if (log) {
		return binMag[plotNumber][index] * 20 * std::log10(index);
	}
	else {
...
float FFTSpectrumAnalyzerAudioProcessorEditor::screenToGraph(float screenCoord) {
	...
	screenCoord = (screenCoord * (xMax - xMin)) / (graphEast - graphWest);
	return screenCoord;
}
...
float FFTSpectrumAnalyzerAudioProcessorEditor::graphToScreen(int graphCoord) {
	...
	graphCoord = (graphCoord * (graphEast - graphWest)) / (xMax - xMin);
